/*
	...
	Optimization:
	- Use local register instead of global memory (points)
	- Use const + __restrict__ to read-only data (clusters)
*/
...
__global__ void clustering(int n, int k, Point* points, const Cluster* __restrict__ clusters) {
	...
	for (int i = 0; i < k; ++i) {
		int dist = (
			(point.x - clusters[i].x) * (point.x - clusters[i].x) + 
			(point.y - clusters[i].y) * (point.y - clusters[i].y) + 
			(point.z - clusters[i].z) * (point.z - clusters[i].z)
		);
	...
	points[idx].cluster = point.cluster;
}
...
__global__ void recentering_phase1(int n, int cluster, Point* points, int* sumX, int* sumY, int* sumZ, int* count) {
	...
	if (point.cluster == cluster) {
		atomicAdd(sumX, point.x);
		atomicAdd(sumY, point.y);
		atomicAdd(sumZ, point.z);
		atomicAdd(count, 1);
	}
}
